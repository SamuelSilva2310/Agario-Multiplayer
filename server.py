...
This the server script all we need to do is run this first,
a server will be setup in our local network so any device that runs the
game in our network will be connected.
...
# Game Variables
player_id = 0  # An ID which will be given to each player.
connections = 0
...
def check_collision(players, blobs):
    """
    Checks player -> blob collision
    @param players: list [{ID: playerObject} , {ID: playerObject},...]
    ...
    for x, player1 in enumerate(sort_players):
        for player2 in sort_players[x + 1:]:
            ...
            if dis < players[player2].size - players[player1].size * 0.85:
                players[player2].size = math.sqrt(
                    players[player2].size ** 2 + players[player1].size ** 2)  # adding areas instead of radii
...
def generate_blobs(blobs):
    """
    Generates blobs so there are always the MAX_BLOB_AMOUNT
    @param blobs: list  [blobObject,blobObject,...]
    @return: None
    """
    if len(blobs) == 0:
...
def player_thread(conn, id):
    """
    A thread method that runs for every player, runs game actions
    @param conn: Object
    @param id: int
    @return: None
    """
    ...
    while True:
        ...
        try:
            # receive any command from a player
            data = conn.recv(32)
